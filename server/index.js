...
app.post("/login", (request, response) => {
  db.query(
    `Select * from students where email= '${request.body.email}' And password= '${request.body.password}' `,
    (err, res) => {
      if (err) {
        throw err;
      } else {
...
app.post("/signup", (request, response) => {
  db.query(
    `Insert Into students (roll_no,username,email,password,depart_id) Values ('${request.body.roll_no}','${request.body.username}','${request.body.email}','${request.body.password}','${request.body.depart_id}')`,
    (err, res) => {
      if (err) {
        throw err;
      } else {
